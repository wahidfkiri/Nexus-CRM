(function () {
  ...
  const els = {
    ...
    boardWorkspace: document.getElementById('trelloBoardWorkspace'),
    ...
    cardForm: document.getElementById('trelloCardForm'),
  ...
  async function jsonRequest(url, options = {}) {
    ...
    if (!response.ok || data.success === false) {
      throw new Error(data.message || 'Action Trello impossible.');
    }
    return data;
  }
    ...
    els.gallery.innerHTML = filtered.map((board) => {
      ...
      return `
        <button type="button" class="ti-board-card ${state.currentBoardId === board.id ? 'is-active' : ''}" data-board-open="${board.id}" style="--board-a:${gradient.primary};--board-b:${gradient.secondary}">
          ...
          <div class="ti-board-card-desc">${escapeHtml(truncate(board.description || 'Board Trello synchronisé dans le CRM.'))}</div>
  ...
  function renderBoardHeader(board) {
    ...
    els.boardHeader.innerHTML = `
      <div class="ti-board-header-main">
        ...
        <p>${escapeHtml(board.description || 'Board Trello synchronisé en local pour une lecture rapide et des interactions plus fluides dans le CRM.')}</p>
    ...
    if (els.currentBoardLabel) {
      els.currentBoardLabel.textContent = board.name;
    }
  ...
  function renderCard(card) {
    ...
    const badges = [];
    if (card.due_at) badges.push(`<span class="ti-card-badge"><i class="fas fa-clock"></i> ${escapeHtml(formatDate(card.due_at))}</span>`);
    if ((card.badges?.comments || 0) > 0) badges.push(`<span class="ti-card-badge"><i class="fas fa-comment"></i> ${card.badges.comments}</span>`);
    if (card.link?.project_name) badges.push(`<span class="ti-card-badge"><i class="fas fa-link"></i> ${escapeHtml(card.link.project_name)}</span>`);

    const cover = card.cover_color ? `<div class="ti-card-cover" style="--cover-a:${card.cover_color};--cover-b:${card.cover_color}"></div>` : '';
    ...
    const linkHtml = card.link?.project_name
      ? `<div class="ti-card-link"><i class="fas fa-diagram-project"></i> Liée au projet ${escapeHtml(card.link.project_name)}</div>`
      : '';
  ...
  function showBoardWorkspace(visible) {
    if (els.boardStage) {
      els.boardStage.style.display = visible ? '' : 'none';
    }
    if (els.boardEmpty) {
      els.boardEmpty.style.display = visible ? 'none' : '';
    }
  }
  ...
  async function loadBoard(boardId, pushState = true) {
    ...
    try {
      ...
      setStatus('Board Trello synchronisé et prêt.', 'success');
    } catch (error) {
  ...
  async function syncAllBoards() {
    ...
    try {
      ...
      setStatus(response.message || 'Synchronisation terminée.', 'success');
      if (window.Toast) {
        window.Toast.success('Trello', response.message || 'Synchronisation terminée.');
      }
  ...
  function clearSelectedBoard() {
    ...
    setStatus('Galerie des boards affichée.', 'info');
  }
  ...
  function openCardModal(cardId) {
    ...
    els.modalMeta.textContent = card.link?.project_name
      ? `Carte liée au projet CRM ${card.link.project_name}`
      : 'Carte Trello non liée à un projet CRM';
  ...
  function populateProjectSelect(selected) {
    ...
    els.cardProject.innerHTML = `<option value="">Aucun projet lié</option>` + state.projects.map((project) => `
  ...
  async function saveCard() {
    ...
    try {
      ...
      if (window.Toast) {
        window.Toast.success('Trello', response.message || 'Carte mise à jour.');
      }
      if (window.Modal) {
        window.Modal.close(els.modal, true);
      }
      await loadBoard(state.currentBoardId, false);
    } catch (error) {
      if (window.Toast) {
  ...
  async function archiveCard() {
    ...
    try {
      ...
      if (window.Toast) {
        window.Toast.success('Trello', response.message || 'Carte archivée.');
      }
      if (window.Modal) {
        window.Modal.close(els.modal, true);
      }
    ...
    els.listsScroller.querySelectorAll('[data-card-form]').forEach((form) => {
      form.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          if (window.Toast) {
            window.Toast.success('Trello', response.message || 'Carte créée.');
          }
          await loadBoard(state.currentBoardId, false);
        } catch (error) {
          if (window.Toast) {
            window.Toast.error('Trello', error.message || 'Impossible de créer la carte.');
          }
        }
      });
    });
    ...
    els.listsScroller.querySelectorAll('[data-list-body]').forEach((zone) => {
      ...
      zone.addEventListener('drop', async (event) => {
        ...
        try {
          ...
          if (window.Toast) {
            window.Toast.success('Trello', response.message || 'Carte déplacée.');
          }
          await loadBoard(state.currentBoardId, false);
        } catch (error) {
          if (window.Toast) {
            window.Toast.error('Trello', error.message || 'Déplacement impossible.');
          }
          await loadBoard(state.currentBoardId, false);
        }
      });
  ...
  function bindGlobalEvents() {
    ...
    els.disconnectBtn?.addEventListener('click', async () => {
      ...
      window.Modal.confirm({
        title: 'Déconnecter Trello',
        message: 'La connexion actuelle sera supprimée du CRM. Vous pourrez reconnecter Trello plus tard.',
        confirmText: 'Déconnecter',
        type: 'danger',
        onConfirm: async () => {
          const response = await jsonRequest(boot.routes.disconnect, { method: 'POST' });
          window.Toast?.success('Trello', response.message || 'Trello déconnecté.');
